...
A noticia de que existem supersalários na Unicamp é antiga, mas voltou a circular recentemente, por conta da decisão do magnífico reitor de publicar a lista nominal -- como exige o Tribunal de Contas do Estado. E já que chutaram o vespeiro, resolvi pegar [o documento que a Diretoria Geral de Recursos Humanos publica][remuneracao-original] e trabalhar um pouco os dados.

## Tirando os dados do PDF e passando para o CSV

Começamos pelo documento, em PDF, péssimo pra trabalhar -- mas no arsenal do Jornalista Hacker do Software Livre temos uma ferramenta ideal para isso: o [Tabula][tabula-upstream]. Instalar no Ubuntu é razoavelmente simples (pros outros sistemas, leia o [README][tabula-readme]):

* [Instale o Java][instalar-java], se não estiver instalado
* Baixe o programa d[o site oficial][tabula-upstream]
* Descompacte e abra a pasta descompactada no terminal
* Execute a linha seguinte:
      java -Dfile.encoding=utf-8 -Xms256M -Xmx1024M -jar tabula.jar

![Fazendo o upload do PDF no Tabula](/assets/img/tabula-import.png)
...
![Selecionando as tabelas no Tabula](/assets/img/tabula-select.png)
...
![Exportando os dados no Tabula](/assets/img/tabula-export.png)

Vai demorar um pouco pra processar todas as páginas. No meu laptop, demorou 4 minutos. Mas eventualmente a tela atualizou e mostrou a opção de baixar [todos os dados como CSV][remuneracao-csv]. Pronto, agora dá pra trabalhar direto com os dados.
...
Agora eu queria ordenar pelo salário bruto e pelo salário líquido, em cópias do arquivo. Eu poderia programar isso com Python puro, mas é tão melhor poder contar com uma solução pronta e empacotada pra gente! Por isso eu resolvi usar o [CSVKit][csvkit-upstream], um canivete suíço pra operar CSV na linha de comando.
...
Se você não tiver um suco para tomar, baixe o [arquivo ordenado por salario bruto][remuneracao-por-bruto] e o [arquivo ordenado por salario liquido][remuneracao-por-liquido].

## Pegando os nomes dos servidores

O PDF original não tinha os nomes, mas o número de matrícula dos servidores públicos. No entanto, a Unicamp tem [um serviço][dgrh-consulta] que permite consultar funcionarios pela matricula. Portanto, foi só questão de escrever um robôzinho pra pegar esses dados usando meu framework favorito, o [Scrapy][scrapy-upstream].

Eu fiz [um projetinho simples][dgrh-scraper], que me permite automatizar a consulta pela matrícula do funcionário. O processo é meio dificultado porque o site nao me permite fazer muitas consultas em paralelo, então eu tive que limitar a apenas uma consulta a cada 7 segundos. O que dá mais ou menos 10 por minuto, no mínmo 10 minutos pra pegar 100 funcionarios ou quase 2 horas pra pegar os mil funcionarios com maior salário. Eu poderia rotear as consultas por outros servidores, pela rede Tor ou por um serviço especializado. Mas eu tenho tempo, posso ir fazer uma janta.
...
Finalmente, preparar os dados pra fazer os plots. Duas transformações em dicionários e basta escrever os CSV: [gráfico dos gastos][sumplot-csv] e [gráfico da contagem de servidores][countplot-csv].
...
Eu fiquei com preguiça de usar o [Matplotlib][matplotlib-upstream], então eu resolvi usar o [Plotly][plotly] manualmente mesmo.

<a href="https://plot.ly/~barraponto/18/" target="_blank" title="Servidores por faixa salarial" style="display: block; text-align: center;"><img src="https://plot.ly/~barraponto/18.png" alt="Servidores por faixa salarial" style="max-width: 100%;width: 1436px;"  width="1436" onerror="this.onerror=null;this.src='https://plot.ly/404.png';" /></a>
<script data-plotly="barraponto:18"  src="https://plot.ly/embed.js" async></script>

<a href="https://plot.ly/~barraponto/26/" target="_blank" title="Gastos por faixa salarial" style="display: block; text-align: center;"><img src="https://plot.ly/~barraponto/26.png" alt="Gastos por faixa salarial" style="max-width: 100%;width: 1436px;"  width="1436" onerror="this.onerror=null;this.src='https://plot.ly/404.png';" /></a>
<script data-plotly="barraponto:26"  src="https://plot.ly/embed.js" async></script>

Fica a pergunta: porque temos tantas pessoas recebendo **menos de um salário mínimo**?

[remuneracao-original]: http://www.dgrh.unicamp.br/remuneracao.pdf
[remuneracao-csv]: /assets/files/tabula-65cdc698f3beab70fb5daa6de6d0d92276872c35.csv
...
[dgrh-consulta]: http://www.siarh.unicamp.br/consultaFuncionario/
[dgrh-scraper]: https://github.com/barraponto/supersalarios
[csvkit-upstream]: http://csvkit.readthedocs.org/en/latest/index.html
[scrapy-upstream]: http://scrapy.org/
[matplotlib-upstream]: http://matplotlib.org/
[plotly]: https://plot.ly/
[tabula-upstream]: http://tabula.technology/
